...
- [x] デコード辞書の拡充 — `82` 規格Version（機器=Release文字+リビジョン / ノードプロファイル=プロトコルVer+電文形式）、`8A` メーカコード（公式一覧から主要社を社名併記、未知は code hex のみ）、電動雨戸・シャッター `0263`（`E0` 開閉動作設定 / `E1` 開度% / `EA` 開閉状態）。辞書になければ生 hex を返す方針は維持。
